...
class Glf_Gallery extends WP_Widget {
		...
		if ( ! empty( $instance['images'] ) ) {
			...
			foreach ( $images as $image ) {
				if ( ! filter_var( $image, FILTER_VALIDATE_URL ) ) {
					if ( $image = wp_get_attachment_image_src( $image, 'full' ) ) {
						$image = $image[0];
					} else {
						$image = false;
					}
				...
				if ( $image ) {
                    ...
                    <a href='{$image}' rel='{$this->id}'>
                        <img src='{$image}' class='img-responsive glf-coverify-image'/>
                    </a>
                </div>";
				}
			}
	...
	public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title:', 'gloriafood-restaurant' ); ?></label>
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"
                   name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text"
                   value="<?php echo esc_attr( $title ); ?>">
        </p>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'subtitle' ) ); ?>"><?php esc_attr_e( 'Subtitle:', 'gloriafood-restaurant' ); ?></label>
